    ...
    @foreach($templates as $template)
        <div class="col-sm-4">
            <div class="card p-1 border shadow-none">
                <div class="p-3">
                    <h5><a href="{{ route('templates.show', $template->id) }}" class="text-dark">{!! $template->header ? \Illuminate\Support\Str::limit(strip_tags($template->header), 15, $end='...') : "Blog has no header" !!}</a></h5>
                    <p class="text-muted mb-0">{{ $template->created_at->format('d M, Y') }}</p>
                </div>
                
                <div class="position-relative image-container">
                    @if ($template->banner)
                        <img src="{{ asset('images/banners/' . $template->banner) }}" alt="{{ $template->banner }}" class="img-thumbnail">
                    @else
                        No photo
                    @endif
                </div>
                ...
                <div class="p-3">
                    ...
                    @if($template->description)
                        {!! $template->description ? \Illuminate\Support\Str::limit(strip_tags($template->description), 19, $end='...') : "No Description" !!}
                    @else
                        No Description 
                    @endif
                    </p>
